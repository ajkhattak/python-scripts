...
class Mesh2D(object):
    def __init__(self, coords, connectivity, labeled_sets=None):
        ...
        print (coords.shape, len(coords.shape))
    ...
    def read_VTK_Unstructured(cls, filename):
        with open(filename,'r') as fid:
            ...
            while True:
                ...
                elif section == 'POLYGONS' or section == 'CELLS':
                    ncells = int(split[1])
    ...
    def from_Transect_MakePieWedge(cls, x, z, arclen, curved = False):
        """Creates a 2D surface slice mesh from transect data"""
        # coordinates
        xlen = len(x)
        if curved is True:
            xarc0 = (x[0] + x[1])* 0.5
    ...
    @classmethod
    def from_Transect_Make3DPie(cls, x, z, radius_pos, curved = False):
        """Creates a 2D surface slice mesh from transect data"""
        # coordinates
        xlen = len(x)
        xmid = radius_pos #79, 39
        x = [round(i,2) for i in x]
        # get all points
        P0 = [x[xmid],0]
        Px = [ 0, x[xmid], x[-1]]
        Py = [0, x[int(xmid/2)], x[xmid] ]
        P = [[Px[0], -Py[1]], [Px[0], Py[1]], [Px[1], Py[2]], [Px[2], Py[1]], [Px[2], -Py[1]], [Px[1], -Py[2]]]
        
        #print P
        Slope = np.zeros(len(P))
        for i,p in enumerate(P):
            #print p
            if p[0] != P0[0]:
                #print p[1]
                s = (P0[1] - p[1] -0.25)/(P0[0] - p[0])
                #s = (5 - p[1]*0 -0.0025)/(P0[0] - p[0])
            else:
                s = 0
            #print s
            Slope[i] = s
        ...
        for i in range(len(x1)):
            c1= P0[1] + Slope[0] *(x1[i] - P0[0])
            c2 = P0[1] + Slope[1] *(x1[i] - P0[0])
            c1 = round(c1, 6)
            c2 = round(c2, 6)
        ...
        y3 = np.linspace(Py[2],0, len(x1))
        y3 = [round(i,4) for i in y3]
        #print y3
        for i in range(len(x3)):
            c4 = P0[1] + Slope[3] * (x3[i] - P0[0])
            c5 = P0[1] + Slope[4] * (x3[i] - P0[0])
            c4 = round(c4, 6)
            c5 = round(c5, 6)
            y4.append(c4)
            y5.append(c5)
        ...
        y5 = y5[::-1]
        y6 = [-i for i in y3]
        x1 = [round(i1,6) for i1 in x1]
        x2 = [round(i1,6) for i1 in x2]
        x3 = [round(i1,6) for i1 in x3]
        z1= z[:xmid+1]
        z3= z[xmid:][::-1]
        z2 = [(i+j)/2. for i,j in zip(z1, z3)]
        

        Xc = np.concatenate((x1, x1, x2, x3, x3, x2, x1))
        Yc = np.concatenate((y1,y2, y3, y4, y5, y6, y1))
        Zc = np.concatenate([z1,z1, z2,z3,z3,z2,z1])

        nsurf_cells = len(x1)-1
        conn = []
        
        nx = len(z[:xmid+1])-1 #nsurf_cells
        ny= 6
        ...
        for j in range(ny):
            for i in range(nx):
                if i < nx -0:
                    ...
                    #print i
                    k = j*(nx+1)
                    conn.append([i + k, i+1 + k, nx + i + 1 + k])
        points = np.array([Xc, Yc, Zc])
        return cls(points.transpose(), conn)


    @classmethod
    def from_Transect_Make3DPie1(cls, x, z, radius_pos, curved = False):
        """Creates a 2D surface slice mesh from transect data"""
        # coordinates
        xlen = len(x)
        xmid = radius_pos #79, 39
        ...
        x = np.array(x)
        x1 = np.array(x[:xmid+1])
        x2 = x1
        xx = [x[xmid] + 0*i for i in x]
        x3 = np.array(xx[:xmid+1])
        x4 = np.array(x[xmid:][::-1])
        x5 = x4
        x6 = np.array(xx[xmid:])
        
        #print len(x1), len(x5), len(x2), len(x4), len(x3),len(x6)
        ...
        for i in range(len(x1)):
            c1 = round(-5.0 + s1*(x1[i] - x[0]),2)
            y1.append(c1)
            y2.append(-c1)
        
        y3 = [ round(x[xmid] - c,2) for c in x1]
        
        y4  =[]
        y5 = []
        for i in range(len(x4)):
            c1 = round(5.0 + s2*(x4[i] - x4[0]),2)
            y4.append(c1)
            y5.append(-c1)
        
        
        y6 = [ round(x[xmid] - c,2) for c in x4]

        
        z1 = z[:xmid+1]
        z2 = z1
        zz = z[::-1]
        z3 = [round((i+j)/2.,2) for i,j in zip(z1,zz[:xmid+1])]
        z4 = zz[:xmid+1]
        z5 = z4
        z6 = [(i+j)/2. for i,j in zip(z4,zz[:xmid+1])]
        

        Xc = np.concatenate((x2, x3, x4, x5))
        Yc = np.concatenate((y2, y3, y4, y5))
        Zc = np.concatenate([z2, z3, z4, z5])
        ...
        Zc = np.concatenate([z1, z2, z3])

        Xc = np.concatenate((x6, x1, x2, x3,x4))
        Yc = np.concatenate((y6, y1, y2, y3,y4))
        Zc = np.concatenate([z6, z1, z2, z3,z4])


        Xc = np.concatenate((x4, x5))
        Yc = np.concatenate((y4, y5))
        Zc = np.concatenate([z4, z4])

        Xc = np.concatenate((x3, x4, x5, x6))
        Yc = np.concatenate((y3, y4, y5,y6))
        Zc = np.concatenate([z3, z4, z5, z6])


        Xc = np.concatenate((x6, x1, x2, x3,x4,x5,x6))
        Yc = np.concatenate((y6, y1, y2, y3,y4,y5,y6))
        Zc = np.concatenate([z6, z1, z2, z3,z4,z5,z6])
        ...
        nx = len(x1)-1 #nsurf_cells
        nx4 = len(x4)-1 #nsurf_cells
        ny= 3
        ...
        for j in range(ny):
            for i in range(nx):
                if i < nx - 1:
                    ...
                    k = j*(nx+1)
                    conn.append([i + k, i+1 + k, nx + i + 1 + k])
        for j in xrange(3,6,1):

            for i in range(nx4):
                if i < nx4 - 1 and j != 5:
                    k = j*(nx4+1)
                    conn.append([i + k, i+1 + k, nx4 + i + 2 + k, nx4 + i + 1 + k])
                elif (j!= 5):
                    k = j*(nx4+1)
                    conn.append([i + k, i+1 + k, nx4 + i + 1 + k])
                elif (j==5):
                    k = j*(nx4+1)
                    conn.append([i, i+1 , nx4 + i + 2 + k, nx4 + i + 1 + k])
